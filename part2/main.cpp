...
class search_problem {
    public:
        ...
        virtual bool search_problem::operator==(const search_problem& other) = 0;
};
...
class satellite_problem : public search_problem {
    ...
    static satellite_problem* get_initial_state(){
        int obs_number = obs_to_do.size();
        return new satellite_problem(0, 1, 3, new vector<bool> (obs_number, false), false);
    };

    static satellite_problem* get_final_state(){
        return new satellite_problem(0, 1, 3, new vector<bool> (obs_number, true), true);
    };
    ...
    bool satellite_problem::operator==(const satellite_problem& other) {
        bool time_eq = this->time == other.is_obs_done;
